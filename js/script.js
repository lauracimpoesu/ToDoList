...
function startProgram() {
    ...
    //Local Storage p. I 
/* nameValue.addEventListener('input', letter => {
    tasksListHTML.textContent = letter.target.value; 
})
 */
    buttonCall();
}
...
function createDisplayMagic() {
        ...
        for (   i=0; i < tasksGroup.length; i++   ) {
            myContentDisplay += "<li class='list-group-item' id='text'>" + tasksGroup[i] + "<p onclick='deleteTask("+i+")' class='deleteButtonOne badge text-secondary'>undo</p></li>";
        }
...
/* var saveToLocalStorage = () => {
    localStorage.setItem('textinput', tasksListHTML.textContent) 
}
 */
